...
def get_hoo_wallet(symbol):
    c =0 
    try:
        url = 'https://etherscan.io/tokenholdingsHandler.aspx?&a=0x0093e5f2a850268c0ca3093c7ea53731296487eb&q='+symbol+'&p=1&f=0&h=1&sort=total_price_usd&order=desc&pUsd24hrs=1234.25&pBtc24hrs=0.0374&pUsd=1230.66&fav=&langMsg=A%20total%20of%20XX%20tokenSS%20found&langFilter=Filtered%20by%20XX&langFirst=First&langPage=Page%20X%20of%20Y&langLast=Last&ps=25'
        data1 = requests.get(url, headers={
            'user-agent': 'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/67.0.3396.99 Safari/537.36'})
        soup = eval(BeautifulSoup(data1.content,'lxml').text)['layout']
        #print(soup)
        a=int(soup.find(symbol, 10)+len(symbol))
        b=int(soup.find('$', a))
        c=float(soup[a:b].replace(',',''))        
    except:
        c = 0 
    return c
...
if __name__ == "__main__":
    ...
    delete_coins=[['ETC','RIF','DOCK'],['BOND'],['HOT','3L','3S','ETC'],['MPH','ETC','COVER','OM','BCHSV'],['RAMP','COVER','CET','ETC','BCHA','KLAY','BOND'],['COVER','AGS','ATP','CET','ETC','OM','BAS','KLAY']]
                        ...
                        if com_pairs !=[]:
                            for s in com_pairs:
                                ...
                                try:
                                    ...
                                    #print(s,profit)
                                    if b_bid_price[0]/a_ask_price[0] > 1.01 and b_bid_price[0]/a_ask_price[0] < 10 and profit >3:
                                        '''
                                        print('-'*60)
                                        print(time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time())),format((b_bid_price[0]/a_ask_price[0]-1)*100,'.3f'),'%','|',exchangeNames[j],'-->',exchangeNames[i],'|',s,'|',format(b_bid_real-a_ask_real,'.1f'),'$')
                                        print('S1: ',b_bid_price[0],'B1: ',a_ask_price[0],'Qty: ',min_vol)
                                        ...
                                        #break                                        
                                    elif a_bid_price[0]/b_ask_price[0] > 1.01 and a_bid_price[0]/b_ask_price[0] <10 and profit >3:
