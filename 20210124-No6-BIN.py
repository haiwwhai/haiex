...
if __name__ == "__main__":
    ...
    delete_coins=[['ETC','RIF','DOCK'],['BOND'],['HOT','3L','3S','ETC'],['MPH','ETC','COVER','SWINGBY','OM','BCHSV'],['RAMP','COVER','CET','ETC','BCHA','KLAY'],['COVER','AGS','ATP','CET','ETC','OM','BAS','KLAY']]
                        ...
                        if com_pairs !=[]:
                            for s in com_pairs:
                                ...
                                try:
                                    ...
                                    #print(s,profit)
                                    if b_bid_price[0]/a_ask_price[0] > 1.01 and profit >3:
                                        '''
                                        print('-'*60)
                                        print(time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time())),format((b_bid_price[0]/a_ask_price[0]-1)*100,'.3f'),'%','|',exchangeNames[j],'-->',exchangeNames[i],'|',s,'|',format(b_bid_real-a_ask_real,'.1f'),'$')
                                        print('S1: ',b_bid_price[0],'B1: ',a_ask_price[0],'Qty: ',min_vol)
                                        ...
                                        #break                                        
                                    elif a_bid_price[0]/b_ask_price[0] > 1.01 and profit >3:
